...
ChatEmp::ChatEmp(QObject *parent)
    : QObject(parent)
    , m_clientSocket(new QTcpSocket(this))
    , m_loggedIn(false)
{
    // Forward the connected and disconnected signals
    connect(m_clientSocket, &QTcpSocket::connected, this, &ChatEmp::connected);
    connect(m_clientSocket, &QTcpSocket::disconnected, this, &ChatEmp::disconnected);
    // connect readyRead() to the slot that will take care of reading the data in
    connect(m_clientSocket, &QTcpSocket::readyRead, this, &ChatEmp::onReadyRead);
    // Forward the error signal, QOverload is necessary as error() is overloaded, see the Qt docs
#if (QT_VERSION < QT_VERSION_CHECK(6, 0, 0))
    connect(m_clientSocket, QOverload<QAbstractSocket::SocketError>::of(&QAbstractSocket::error), this, &ChatEmp::error);
#else
...
void ChatEmp::login(const QString &userName)
{
...
void ChatEmp::sendMessage(const QString &text)
{
    if (text.isEmpty())
...
void ChatEmp::disconnectFromHost()
{
    m_clientSocket->disconnectFromHost();
}

void ChatEmp::jsonReceived(const QJsonObject &docObj)
{
...
void ChatEmp::connectToServer(const QHostAddress &address, quint16 port)
{
    m_clientSocket->connectToHost(address, port);
}

void ChatEmp::onReadyRead()
{
