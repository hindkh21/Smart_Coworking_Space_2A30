...
MyChat_employe::MyChat_employe(QWidget *parent)
    : QWidget(parent)
    , ui(new Ui::MyChat_employe) // create the elements defined in the .ui file
    , m_chatEmploye(new ChatEmp(this)) // create the chat client
    , m_chatModel(new QStandardItemModel(this)) // create the model to hold the messages
{
    ...
    connect(m_chatEmploye, &ChatEmp::connected, this, &MyChat_employe::connectedToServer);
    connect(m_chatEmploye, &ChatEmp::loggedIn, this, &MyChat_employe::loggedIn);
    connect(m_chatEmploye, &ChatEmp::loginError, this, &MyChat_employe::loginFailed);
    connect(m_chatEmploye, &ChatEmp::messageReceived, this, &MyChat_employe::messageReceived);
    connect(m_chatEmploye, &ChatEmp::disconnected, this, &MyChat_employe::disconnectedFromServer);
    connect(m_chatEmploye, &ChatEmp::error, this, &MyChat_employe::error);
    connect(m_chatEmploye, &ChatEmp::userJoined, this, &MyChat_employe::userJoined);
    connect(m_chatEmploye, &ChatEmp::userLeft, this, &MyChat_employe::userLeft);
    // connect the connect button to a slot that will attempt the connection
    connect(ui->connectButton, &QPushButton::clicked, this, &MyChat_employe::attemptConnection);
    // connect the click of the "send" button and the press of the enter while typing to the slot that sends the message
    connect(ui->sendButton, &QPushButton::clicked, this, &MyChat_employe::sendMessage);
    connect(ui->messageEdit, &QLineEdit::returnPressed, this, &MyChat_employe::sendMessage);
}
MyChat_employe::~MyChat_employe()
{
    // delete the elements created from the .ui file
    delete ui;
}

void MyChat_employe::attemptConnection()          //Localhost & port
{
    ...
    m_chatEmploye->connectToServer(QHostAddress(hostAddress), 1967);
}

void MyChat_employe::connectedToServer()   //Connect with server
{
    ...
    if (newUsername.isEmpty()){
        ...
        return m_chatEmploye->disconnectFromHost();
    }
    // try to login with the given username
    attemptLogin(newUsername);
}

void MyChat_employe::attemptLogin(const QString &userName)  //prendre Login = username
{
    // use the client to attempt a log in with the given username
    m_chatEmploye->login(userName);
}

void MyChat_employe::loggedIn()
{
...
void MyChat_employe::loginFailed(const QString &reason)   //si servor off
{
...
void MyChat_employe::messageReceived(const QString &sender, const QString &text)
{
...
void MyChat_employe::sendMessage()
{
    // we use the client to send the message that the user typed
    m_chatEmploye->sendMessage(ui->messageEdit->text());
...
void MyChat_employe::disconnectedFromServer()
{
...
void MyChat_employe::userJoined(const QString &username)
{
    // store the index of the new row to append to the model containing the messages
    const int newRow = m_chatModel->rowCount();
    // insert a row
...
void MyChat_employe::userLeft(const QString &username)
{
    // store the index of the new row to append to the model containing the messages
    const int newRow = m_chatModel->rowCount();
    // insert a row
